 ...
 */
int
file_read_int16_t (FILE *fp, int16_t *buf, long count) {
	while (count-- > 0) {
		uint8_t c1 = getc(fp);
		uint8_t c2 = getc(fp);
		*buf = c1 | (c2 << 8);
 ...
 */
int
file_read_int32_t (FILE *fp, int32_t *buf, long count) {
	while (count-- > 0) {
		uint8_t c[4];
		*buf = 0;
 ...
 */
long
file_read_vbytes (FILE *fp, void *buf, long count) {
 ...
 */
int
file_read_bytes (FILE *fp, void *buf, long count) {
 ...
 */
void
file_write_int16_t (FILE *fd, int16_t buf) {
	putc (       buf & 0xff, fd);
	putc ((buf >> 8) & 0xff, fd);
}
 ...
 */
void
file_write_int32_t (FILE *fd, int32_t buf, long count) {
...
MDirPtr
FindMasterDir (MDirPtr from, const char *name) {
	while (from) {
 ...
 */
MDirPtr
FindMasterDir (MDirPtr from, const char *name1, const char *name2) {
	while (from) {
...
void
WriteBytes (FILE *file, const void *buf, long size) {
	if (!Registered)
		return;
	while (size > 0x8000) {
 ...
 */
int
copy_bytes (FILE *dest, FILE *source, size_t size) {
	int          rc      = 0;
	void        *data    = 0;
	const size_t chunksz = 0x4000;

	data = malloc(chunksz + 2);
	while (size > chunksz) {
