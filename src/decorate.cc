...
{	// This needs a FIX
	...
	sprintf(filename,"/tmp/decorate%d",rand());
	...
	while (fgets (readbuf, sizeof readbuf, tempfile))
   	{	current = 0;
		...
		}else if ((! y_strnicmp(token[0], "renderstyle",11)) && ((! y_strnicmp(token[1], "optfuzzy",8)) || (! y_strnicmp(token[1], "fuzzy",5))))
		{	buf.flags 	= 's';
		}
	...
	// if things were found inside the decorate file,add these now to a group
