 ...
 */
Entry2_action_t
Entry2::process_event (const input_status_t &is) {
	if (is.key == YE_EXPOSE) {
		background_drawn = false;
		return ACT_NONE;
	}
 ...
 */
int
Entry2::count_widgets (const char *fmt) {
	int nwidgets = 0;
 ...
 */
int
Entry2::fill_in_widgets_info (const char *fmt, va_list args) {
	size_t f = 0;
 ...
 */
void
Entry2::jump_to_field (size_t field_no) {
	if (field_no >= nfields) {
